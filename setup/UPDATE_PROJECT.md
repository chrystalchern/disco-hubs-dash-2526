...
5. **Reboot when changes do not show on public app**: A reboot is always needed when the environment is modified; e.g., when an update is made to `environment.yml`. Sometimes, a reboot is needed for code changes. To reboot the app, the owner of the app must follow [these instructions from Streamlit](https://docs.streamlit.io/deploy/streamlit-community-cloud/manage-your-app/reboot-your-app).